  ...
  : WApplication(env)
{
  ...
  EventDisplayer *evd= new EventDisplayer(0);
  ...
  WMenu *menu = new WMenu(contentsStack, Vertical, 0);
  menu->setRenderAsList(true);
  menu->setStyleClass("menu");
  menu->setInternalPathEnabled();
  menu->setInternalBasePath("/");

  FormWidgets *fw = new FormWidgets(evd);
  WSubMenuItem *smi = new WSubMenuItem("Form Widgets", fw);
  WMenu *subMenu = new WMenu(contentsStack, Vertical, 0);
  ...
  fw->populateSubMenu(subMenu);

  root()->addWidget(menu);                               // insert the form widget menu
  root()->addWidget(new WBreak());                       // insert a line break
  root()->addWidget(contentsStack);
  root()->addWidget(evd);                                // insert a line break
  root()->addWidget(new WBreak());                       // insert a line break
